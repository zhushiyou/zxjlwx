...
Page({
    ...
    loadData: function(o) {
        var p = this, g = getApp().core.getStorageSync(getApp().const.ITEM);
        p.setData({
            total: void 0 !== g.total ? g.total : {
                total_num: 0,
                total_price: 0
            },
            carGoods: void 0 !== g.carGoods ? g.carGoods : []
            ...
            success: function(o) {
                if (getApp().core.hideLoading(), 0 == o.code) {
                    ...
                    for (var i in t) if (0 < t[i].goods.length) for (var c in e.push(t[i]), t[i].goods) {
                        ...
                        n) {
                            var d = p.data.carGoods;
                            for (var r in d) g.carGoods[r].goods_id === parseInt(t[i].goods[c].id) && (t[i].goods[c].num = t[i].goods[c].num ? t[i].goods[c].num : 0, 
                            t[i].goods[c].num += g.carGoods[r].num);
                            parseInt(t[i].goods[c].hot_cakes) && a.push(t[i].goods[c]), s.push(t[i].goods[c].id);
                        }
                    }
                    p.setData({
